...
void swap(int i, int j){
    SWCharacter aux = lista[i];
    lista[i] = lista[j];
    lista[j] = aux;
}

void quicksortRec(SWCharacter *array, int esq, int dir) {
    int i = esq, j = dir;
    SWCharacter pivo = array[(dir+esq)/2];
    while (i <= j) {
        while (strcmp(array[i].hair_color, pivo.hair_color) < 0) i++;
        while (strcmp(array[i].hair_color, pivo.hair_color) > 0) j--;
        if (i <= j) {
            swap(i , j);
            i++;
            j--;
...
void quicksort(SWCharacter *array, int n) {
